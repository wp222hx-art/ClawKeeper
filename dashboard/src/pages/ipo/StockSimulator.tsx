...
//              ipo_stock_price_simulator agent through the global runs
//              context. The AI commentary task survives page navigation; the
//              slot is locked while the agent is running.
...
import { ArrowLeft, LineChart, Loader2, Play, X } from 'lucide-react';
...
export function IpoStockSimulator() {
  ...
  // Bind the AI commentary slot. Survives page navigation; one analysis at
  // a time per project.
  const slot_key = id ? slot_keys.stock_sim(id) : '';
  const ai_run = use_slot_run({
    agent_id:   SIM_AGENT_ID,
    slot_key,
    project_id: id,
    module:     'stock_sim',
    label:      t('sim.ai.title'),
  });
  ...
  const ask_ai = async () => {
    ...
    await ai_run.start({ prompt, locale });
  };
  ...
  return (
    <div className="p-6 space-y-6 max-w-6xl mx-auto">
      ...
      <Card className={ai_run.is_completed ? 'border-primary/40' : ''}>
        <CardHeader>
        ...
        <CardContent className="space-y-3">
          <div className="flex items-center gap-2">
            <Button onClick={ask_ai} disabled={ai_run.is_running || !id}>
              {ai_run.is_running ? <Loader2 className="h-3.5 w-3.5 mr-1 animate-spin" /> : <Play className="h-3.5 w-3.5 mr-1" />}
              {ai_run.is_running ? `${t('agents.run.button_running')} (${ai_run.elapsed_s}s)` : t('sim.ai.button')}
            </Button>
            {ai_run.is_running && (
              <Button size="sm" variant="ghost" onClick={() => { void ai_run.cancel(); }}>
                <X className="h-3.5 w-3.5 mr-1" /> 取消
              </Button>
            )}
          </div>
          {ai_run.is_failed && ai_run.current?.error?.message && (
            <div className="text-xs rounded border border-destructive/40 bg-destructive/10 p-2 text-destructive font-mono">
              {ai_run.current.error.message}
            </div>
          )}
          {ai_run.is_completed && ai_run.result && (
            <div className="space-y-2">
              <div className="flex flex-wrap items-center gap-2 text-xs">
                <Badge variant={ai_run.result.mode === 'live' ? 'default' : 'outline'}>
                  {ai_run.result.mode === 'live' ? t('agents.run.mode_live') : t('agents.run.mode_dry_run')}
                </Badge>
                <span className="text-muted-foreground">
                  {t('agents.run.provider_label')}: <code>{ai_run.result.provider}</code> ·{' '}
                  {t('agents.run.model_label')}: <code>{ai_run.result.model}</code> ·{' '}
                  {ai_run.result.ms_elapsed}ms
                </span>
              </div>
              <pre className="text-xs whitespace-pre-wrap p-3 rounded border bg-muted/30 max-h-72 overflow-y-auto">
                {ai_run.result.output}
              </pre>
            </div>
          )}
        </CardContent>
