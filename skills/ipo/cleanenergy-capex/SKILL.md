# SKILL — cleanenergy-capex

> Bound to agent: `cleanenergy_capex_analyzer` (Clean Energy Capex Analyzer)

## When to use

Invoked by `cleanenergy_capex_analyzer` whenever its capabilities are routed:
- `capex_analysis`
...
interface CleanenergyCapexInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface CleanenergyCapexOutput {
  summary: string;
  findings: Array<{
    category: string;
