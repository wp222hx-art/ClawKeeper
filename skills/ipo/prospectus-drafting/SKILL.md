# SKILL — prospectus-drafting

> Bound to agent: `ipo_disclosure_lead` (Disclosure & Drafting Lead)

## When to use

Invoked by `ipo_disclosure_lead` whenever its capabilities are routed:
- `document_drafting`
- `comment_response`
- `narrative_design`
...
interface ProspectusDraftingInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface ProspectusDraftingOutput {
  summary: string;
  findings: Array<{
    category: string;
