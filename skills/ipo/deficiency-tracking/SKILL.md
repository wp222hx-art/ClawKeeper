...
> Bound to agent: `ipo_deficiency_tracker` (Deficiency Tracker)

## When to use

Invoked by `ipo_deficiency_tracker` whenever its capabilities are routed:
- `deficiency_tracking`
...
interface DeficiencyTrackingInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface DeficiencyTrackingOutput {
  summary: string;
  findings: Array<{
    category: string;
