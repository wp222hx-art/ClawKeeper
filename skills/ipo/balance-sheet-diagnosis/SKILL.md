# SKILL — balance-sheet-diagnosis

> Bound to agent: `ipo_balance_sheet_diagnostician` (Balance Sheet Diagnostician)

## When to use

Invoked by `ipo_balance_sheet_diagnostician` whenever its capabilities are routed:
...
interface BalanceSheetDiagnosisInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface BalanceSheetDiagnosisOutput {
  summary: string;
  findings: Array<{
    category: string;
