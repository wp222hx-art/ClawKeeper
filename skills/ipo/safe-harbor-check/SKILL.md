# SKILL — safe-harbor-check

> Bound to agent: `sec_safe_harbor_checker` (Safe Harbor Checker)

## When to use

Invoked by `sec_safe_harbor_checker` whenever its capabilities are routed:
- `safe_harbor_check`
...
interface SafeHarborCheckInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface SafeHarborCheckOutput {
  summary: string;
  findings: Array<{
    category: string;
