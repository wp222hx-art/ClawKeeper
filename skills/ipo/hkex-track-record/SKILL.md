# SKILL — hkex-track-record

> Bound to agent: `hkex_track_record_validator` (HKEX Track Record Validator)

## When to use

Invoked by `hkex_track_record_validator` whenever its capabilities are routed:
...
interface HkexTrackRecordInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface HkexTrackRecordOutput {
  summary: string;
  findings: Array<{
    category: string;
