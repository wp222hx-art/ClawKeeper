...
> Bound to agent: `ipo_cash_flow_analyst` (Cash Flow Analyst)

## When to use

Invoked by `ipo_cash_flow_analyst` whenever its capabilities are routed:
- `cash_flow_analysis`
...
interface CashFlowAnalysisInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface CashFlowAnalysisOutput {
  summary: string;
  findings: Array<{
    category: string;
