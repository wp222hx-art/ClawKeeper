# SKILL — hkex-qa-response

> Bound to agent: `hkex_sponsor_qa_handler` (HKEX Sponsor Q&A Handler)

## When to use

Invoked by `hkex_sponsor_qa_handler` whenever its capabilities are routed:
- `hkex_qa_response`
...
```ts
interface HkexQaResponseOutput {
  summary: string;
  findings: Array<{
    category: string;
