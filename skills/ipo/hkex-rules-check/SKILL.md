# SKILL — hkex-rules-check

> Bound to agent: `hkex_chapter_checker` (HKEX Main Board Rules Checker)

## When to use

Invoked by `hkex_chapter_checker` whenever its capabilities are routed:
- `main_board_rules_check`

## Inputs (schema)

```ts
// TODO: tighten this contract per skill
interface HkexRulesCheckInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface HkexRulesCheckOutput {
  summary: string;
  findings: Array<{
    category: string;
