# SKILL — control-monitoring

> Bound to agent: `ipo_internal_control_monitor` (Internal Control Monitor)

## When to use

Invoked by `ipo_internal_control_monitor` whenever its capabilities are routed:
- `control_monitoring`
...
interface ControlMonitoringInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface ControlMonitoringOutput {
  summary: string;
  findings: Array<{
    category: string;
