# SKILL — amendment-planning

> Bound to agent: `ipo_amendment_planner` (Amendment Planner)

## When to use

Invoked by `ipo_amendment_planner` whenever its capabilities are routed:
- `amendment_planning`
...
interface AmendmentPlanningInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface AmendmentPlanningOutput {
  summary: string;
  findings: Array<{
    category: string;
