# SKILL — lockup-tracking

> Bound to agent: `ipo_lockup_period_tracker` (Lockup Period Tracker)

## When to use

Invoked by `ipo_lockup_period_tracker` whenever its capabilities are routed:
- `lockup_tracking`
...
```ts
interface LockupTrackingOutput {
  summary: string;
  findings: Array<{
    category: string;
