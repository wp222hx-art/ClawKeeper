# SKILL — quarterly-filing

> Bound to agent: `ipo_quarterly_filing_assistant` (Quarterly Filing Assistant)

## When to use

Invoked by `ipo_quarterly_filing_assistant` whenever its capabilities are routed:
- `quarterly_filing`
...
interface QuarterlyFilingInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface QuarterlyFilingOutput {
  summary: string;
  findings: Array<{
    category: string;
