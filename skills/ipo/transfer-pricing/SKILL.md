...
> Bound to agent: `ipo_transfer_pricing_advisor` (Transfer Pricing Advisor)

## When to use

Invoked by `ipo_transfer_pricing_advisor` whenever its capabilities are routed:
- `transfer_pricing`
...
```ts
interface TransferPricingOutput {
  summary: string;
  findings: Array<{
    category: string;
