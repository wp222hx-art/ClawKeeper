# SKILL — cleanenergy-subsidy

> Bound to agent: `cleanenergy_subsidy_tracker` (Clean Energy Subsidy Tracker)

## When to use

Invoked by `cleanenergy_subsidy_tracker` whenever its capabilities are routed:
- `subsidy_tracking`
...
interface CleanenergySubsidyInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface CleanenergySubsidyOutput {
  summary: string;
  findings: Array<{
    category: string;
