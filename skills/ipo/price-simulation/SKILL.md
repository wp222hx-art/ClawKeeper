# SKILL — price-simulation

> Bound to agent: `ipo_stock_price_simulator` (Stock Price Simulator)

## When to use

Invoked by `ipo_stock_price_simulator` whenever its capabilities are routed:
- `price_simulation`
...
```ts
interface PriceSimulationOutput {
  summary: string;
  findings: Array<{
    category: string;
