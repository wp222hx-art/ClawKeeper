# SKILL — listing-standards-match

> Bound to agent: `sec_listing_standards_matcher` (SEC Listing Standards Matcher)

## When to use

Invoked by `sec_listing_standards_matcher` whenever its capabilities are routed:
- `listing_standards_match`

## Inputs (schema)

```ts
// TODO: tighten this contract per skill
interface ListingStandardsMatchInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface ListingStandardsMatchOutput {
  summary: string;
  findings: Array<{
    category: string;
