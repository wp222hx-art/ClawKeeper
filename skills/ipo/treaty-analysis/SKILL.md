# SKILL — treaty-analysis

> Bound to agent: `ipo_treaty_analyzer` (Tax Treaty Analyzer)

## When to use

Invoked by `ipo_treaty_analyzer` whenever its capabilities are routed:
- `treaty_analysis`
...
```ts
interface TreatyAnalysisOutput {
  summary: string;
  findings: Array<{
    category: string;
