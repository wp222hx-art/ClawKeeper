# SKILL — lockup-analysis

> Bound to agent: `ipo_lockup_clause_analyzer` (Lockup Clause Analyzer)

## When to use

Invoked by `ipo_lockup_clause_analyzer` whenever its capabilities are routed:
- `lockup_analysis`
...
```ts
interface LockupAnalysisOutput {
  summary: string;
  findings: Array<{
    category: string;
