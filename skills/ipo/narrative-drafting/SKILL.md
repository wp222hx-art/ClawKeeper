# SKILL — narrative-drafting

> Bound to agent: `ipo_investor_narrative_drafter` (Investor Narrative Drafter)

## When to use

Invoked by `ipo_investor_narrative_drafter` whenever its capabilities are routed:
- `narrative_drafting`
...
interface NarrativeDraftingInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface NarrativeDraftingOutput {
  summary: string;
  findings: Array<{
    category: string;
