# SKILL — hkex-connected-tx

> Bound to agent: `hkex_connected_tx_analyzer` (HKEX Connected Transaction Analyzer)

## When to use

Invoked by `hkex_connected_tx_analyzer` whenever its capabilities are routed:
- `connected_tx_analysis`

## Inputs (schema)

```ts
// TODO: tighten this contract per skill
interface HkexConnectedTxInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface HkexConnectedTxOutput {
  summary: string;
  findings: Array<{
    category: string;
