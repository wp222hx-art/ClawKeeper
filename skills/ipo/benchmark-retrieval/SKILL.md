# SKILL — benchmark-retrieval

> Bound to agent: `ipo_industry_benchmark_retriever` (Industry Benchmark Retriever)

## When to use

Invoked by `ipo_industry_benchmark_retriever` whenever its capabilities are routed:
- `benchmark_retrieval`
...
interface BenchmarkRetrievalInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface BenchmarkRetrievalOutput {
  summary: string;
  findings: Array<{
    category: string;
