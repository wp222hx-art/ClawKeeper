# SKILL — biopharma-clinical-disclosure

> Bound to agent: `biopharma_clinical_disclosure_drafter` (BioPharma Clinical Disclosure Drafter)

## When to use

Invoked by `biopharma_clinical_disclosure_drafter` whenever its capabilities are routed:
- `clinical_disclosure`
...
interface BiopharmaClinicalDisclosureInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface BiopharmaClinicalDisclosureOutput {
  summary: string;
  findings: Array<{
    category: string;
