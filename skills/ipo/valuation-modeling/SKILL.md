...
> Bound to agent: `ipo_valuation_lead` (Valuation Lead)

## When to use

Invoked by `ipo_valuation_lead` whenever its capabilities are routed:
- `dcf_modeling`
- `comp_analysis`
...
interface ValuationModelingInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface ValuationModelingOutput {
  summary: string;
  findings: Array<{
    category: string;
