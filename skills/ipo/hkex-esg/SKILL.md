...
> Bound to agent: `hkex_esg_disclosure_drafter` (HKEX ESG Disclosure Drafter)

## When to use

Invoked by `hkex_esg_disclosure_drafter` whenever its capabilities are routed:
- `esg_disclosure_drafting`

## Inputs (schema)

```ts
// TODO: tighten this contract per skill
interface HkexEsgInput {
  project_id: string;
  tenant_id: string;
