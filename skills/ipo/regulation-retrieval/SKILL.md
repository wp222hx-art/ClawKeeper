# SKILL — regulation-retrieval

> Bound to agent: `ipo_regulation_retriever` (Regulation Retriever)

## When to use

Invoked by `ipo_regulation_retriever` whenever its capabilities are routed:
...
interface RegulationRetrievalInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface RegulationRetrievalOutput {
  summary: string;
  findings: Array<{
    category: string;
