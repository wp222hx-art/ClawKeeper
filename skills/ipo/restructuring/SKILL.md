...
> Bound to agent: `ipo_restructuring_advisor` (Restructuring Advisor)

## When to use

Invoked by `ipo_restructuring_advisor` whenever its capabilities are routed:
- `restructuring_advisory`

## Inputs (schema)

```ts
// TODO: tighten this contract per skill
interface RestructuringInput {
  project_id: string;
  tenant_id: string;
