# SKILL — cleanenergy-supply-chain

> Bound to agent: `cleanenergy_supply_chain_analyzer` (Clean Energy Supply Chain Analyzer)

## When to use

Invoked by `cleanenergy_supply_chain_analyzer` whenever its capabilities are routed:
...
interface CleanenergySupplyChainInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface CleanenergySupplyChainOutput {
  summary: string;
  findings: Array<{
    category: string;
