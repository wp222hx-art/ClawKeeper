# SKILL — industry-routing

> Bound to agent: `ipo_industry_analysis_lead` (Industry Analysis Lead)

## When to use

Invoked by `ipo_industry_analysis_lead` whenever its capabilities are routed:
- `industry_routing`
...
interface IndustryRoutingInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface IndustryRoutingOutput {
  summary: string;
  findings: Array<{
    category: string;
