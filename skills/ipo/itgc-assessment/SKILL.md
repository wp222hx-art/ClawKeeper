# SKILL — itgc-assessment

> Bound to agent: `ipo_itgc_assessor` (IT General Controls Assessor)

## When to use

Invoked by `ipo_itgc_assessor` whenever its capabilities are routed:
- `itgc_assessment`
...
```ts
interface ItgcAssessmentOutput {
  summary: string;
  findings: Array<{
    category: string;
