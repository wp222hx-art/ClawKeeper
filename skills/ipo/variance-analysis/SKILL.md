...
> Bound to agent: `ipo_variance_analyst` (Variance Analyst)

## When to use

Invoked by `ipo_variance_analyst` whenever its capabilities are routed:
- `variance_analysis`
...
interface VarianceAnalysisInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface VarianceAnalysisOutput {
  summary: string;
  findings: Array<{
    category: string;
