# SKILL — walkthrough-documentation

> Bound to agent: `ipo_walkthrough_documenter` (Walkthrough Documenter)

## When to use

Invoked by `ipo_walkthrough_documenter` whenever its capabilities are routed:
...
interface WalkthroughDocumentationInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface WalkthroughDocumentationOutput {
  summary: string;
  findings: Array<{
    category: string;
