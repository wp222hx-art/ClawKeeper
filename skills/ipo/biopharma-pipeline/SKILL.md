# SKILL — biopharma-pipeline

> Bound to agent: `biopharma_pipeline_evaluator` (BioPharma Pipeline Evaluator)

## When to use

Invoked by `biopharma_pipeline_evaluator` whenever its capabilities are routed:
- `pipeline_evaluation`
...
interface BiopharmaPipelineInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface BiopharmaPipelineOutput {
  summary: string;
  findings: Array<{
    category: string;
