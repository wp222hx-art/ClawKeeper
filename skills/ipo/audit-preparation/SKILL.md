...
> Bound to agent: `ipo_audit_lead` (Audit Preparation Lead)

## When to use

Invoked by `ipo_audit_lead` whenever its capabilities are routed:
- `audit_planning`
...
interface AuditPreparationInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface AuditPreparationOutput {
  summary: string;
  findings: Array<{
    category: string;
