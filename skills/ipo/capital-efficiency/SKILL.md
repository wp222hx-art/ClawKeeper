# SKILL — capital-efficiency

> Bound to agent: `ipo_capital_efficiency_analyzer` (Capital Efficiency Analyzer)

## When to use

Invoked by `ipo_capital_efficiency_analyzer` whenever its capabilities are routed:
- `capital_efficiency`
...
interface CapitalEfficiencyInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface CapitalEfficiencyOutput {
  summary: string;
  findings: Array<{
    category: string;
