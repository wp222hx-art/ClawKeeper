# SKILL — related-party-review

> Bound to agent: `ipo_related_party_tx_reviewer` (Related Party Tx Reviewer)

## When to use

Invoked by `ipo_related_party_tx_reviewer` whenever its capabilities are routed:
...
interface RelatedPartyReviewInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface RelatedPartyReviewOutput {
  summary: string;
  findings: Array<{
    category: string;
