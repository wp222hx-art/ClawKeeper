# SKILL — legal-structuring

> Bound to agent: `ipo_legal_lead` (Legal Structuring Lead)

## When to use

Invoked by `ipo_legal_lead` whenever its capabilities are routed:
- `legal_structuring`
...
interface LegalStructuringInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface LegalStructuringOutput {
  summary: string;
  findings: Array<{
    category: string;
