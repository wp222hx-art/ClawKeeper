# SKILL — jurisdiction-routing

> Bound to agent: `ipo_jurisdiction_lead` (Jurisdiction Compliance Lead)

## When to use

Invoked by `ipo_jurisdiction_lead` whenever its capabilities are routed:
...
interface JurisdictionRoutingInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface JurisdictionRoutingOutput {
  summary: string;
  findings: Array<{
    category: string;
