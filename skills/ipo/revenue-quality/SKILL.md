# SKILL — revenue-quality

> Bound to agent: `ipo_revenue_quality_analyzer` (Revenue Quality Analyzer)

## When to use

Invoked by `ipo_revenue_quality_analyzer` whenever its capabilities are routed:
- `revenue_quality`
...
```ts
interface RevenueQualityOutput {
  summary: string;
  findings: Array<{
    category: string;
