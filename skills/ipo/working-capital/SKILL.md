...
> Bound to agent: `ipo_working_capital_analyzer` (Working Capital Analyzer)

## When to use

Invoked by `ipo_working_capital_analyzer` whenever its capabilities are routed:
- `working_capital`
...
```ts
interface WorkingCapitalOutput {
  summary: string;
  findings: Array<{
    category: string;
