...
> Bound to agent: `ipo_financial_diagnosis_lead` (Financial Diagnosis Lead)

## When to use

Invoked by `ipo_financial_diagnosis_lead` whenever its capabilities are routed:
- `financial_diagnosis`
- `finding_aggregation`
...
interface FinancialDiagnosisInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface FinancialDiagnosisOutput {
  summary: string;
  findings: Array<{
    category: string;
