# SKILL — workpaper-organization

> Bound to agent: `ipo_workpaper_organizer` (Workpaper Organizer)

## When to use

Invoked by `ipo_workpaper_organizer` whenever its capabilities are routed:
- `workpaper_organization`

## Inputs (schema)

```ts
// TODO: tighten this contract per skill
interface WorkpaperOrganizationInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface WorkpaperOrganizationOutput {
  summary: string;
  findings: Array<{
    category: string;
