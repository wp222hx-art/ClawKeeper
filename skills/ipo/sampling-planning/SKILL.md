# SKILL — sampling-planning

> Bound to agent: `ipo_sampling_planner` (Sampling Planner)

## When to use

Invoked by `ipo_sampling_planner` whenever its capabilities are routed:
- `sampling_planning`
...
interface SamplingPlanningInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface SamplingPlanningOutput {
  summary: string;
  findings: Array<{
    category: string;
