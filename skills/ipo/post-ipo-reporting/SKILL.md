# SKILL — post-ipo-reporting

> Bound to agent: `ipo_post_ipo_lead` (Post-IPO Lead)

## When to use

Invoked by `ipo_post_ipo_lead` whenever its capabilities are routed:
- `quarterly_reporting`
- `lockup_tracking`
- `price_simulation`
...
interface PostIpoReportingInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface PostIpoReportingOutput {
  summary: string;
  findings: Array<{
    category: string;
