# SKILL — business-section

> Bound to agent: `ipo_business_section_drafter` (Business Section Drafter)

## When to use

Invoked by `ipo_business_section_drafter` whenever its capabilities are routed:
- `business_section_drafting`

## Inputs (schema)

```ts
// TODO: tighten this contract per skill
interface BusinessSectionInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface BusinessSectionOutput {
  summary: string;
  findings: Array<{
    category: string;
