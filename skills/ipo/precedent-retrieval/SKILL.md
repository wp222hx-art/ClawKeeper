...
> Bound to agent: `ipo_precedent_case_retriever` (Precedent Case Retriever)

## When to use

Invoked by `ipo_precedent_case_retriever` whenever its capabilities are routed:
- `precedent_retrieval`
...
interface PrecedentRetrievalInput {
  project_id: string;
  tenant_id: string;
...
```ts
interface PrecedentRetrievalOutput {
  summary: string;
  findings: Array<{
    category: string;
